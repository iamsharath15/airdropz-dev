...
const StoryCarousel: React.FC<StoryCarouselProps> = ({ stories }) => {
  ...
  useEffect(() => {
    if (!selectedStory || !isPlaying) return clearInterval(intervalRef.current!);
            ...
            <div className="relative flex flex-col items-center">
              ...
              <div className="flex justify-between items-center p-4 w-full">
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); togglePlayPause(); }} className="text-gray-400 hover:text-white">
                    {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                  </Button>
                  <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); closeStory(); }} className="text-gray-400 hover:text-white">
                    <X className="w-4 h-4" />
                  </Button>
                </div>
              </div>
              ...
              {/* Navigation Arrows */}
              <div className="absolute left-0 w-1/12 h-full z-10 cursor-pointer flex items-center justify-start px-2" onClick={(e) => { e.stopPropagation(); goToPrev(); }}>
                <ChevronLeft className="w-6 h-6 text-white" />
              </div>
              <div className="absolute right-0 w-1/12 h-full z-10 cursor-pointer flex items-center justify-end px-2" onClick={(e) => { e.stopPropagation(); goToNext(); }}>
